...
export class PhysicsWorld {
  ...
  public createPhysicsObject(type: PhysicsShapeType, config: PhysicsObjectConfig): RAPIER.Collider {
    switch (type) {
      ...
      case PhysicsShapeType.RECTANGLE:
        if (!config.width || !config.height) throw new Error('Width and height are required for rectangle');
        return this.createDynamicRectangle(config.x, config.y, config.width, config.height);
        
      case PhysicsShapeType.STATIC_RECTANGLE:
        if (!config.width || !config.height) throw new Error('Width and height are required for static rectangle');
        return this.createStaticRectangle(config.x, config.y, config.width, config.height);
  ...
  public createPhysicsSphere(x: number, y: number, size: number): RAPIER.Collider {
    return this.createSphere(x, y, size);
  }

  public createPhysicsRect(x: number, y: number, width: number, height: number): RAPIER.Collider {
    return this.createDynamicRectangle(x, y, width, height);
  }

  public createStaticPhysicsRect(x: number, y: number, width: number, height: number): RAPIER.Collider {
  ...
  private createDynamicRectangle(x: number, y: number, width: number, height: number): RAPIER.Collider {
    const rigidBodyDesc = RAPIER.RigidBodyDesc.dynamic().setTranslation(x, y);
    const rigidBody = this.physicsWorld.createRigidBody(rigidBodyDesc);
  ...
  private createStaticRectangle(x: number, y: number, width: number, height: number): RAPIER.Collider {
  ...
  private scheduleKinematicConversion(rigidBody: RAPIER.RigidBody, delay: number): void {
    setTimeout(() => {
  ...
  public contactPair(collider1: RAPIER.Collider, collider2: RAPIER.Collider, callback: () => void): void {
